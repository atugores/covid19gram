...
def botons(taula,dades="casos",regio="Total"):
    ibt=[]
    l_botns =[]
    botonets = []
    result_all = []
...
def b_alphabet(taula,dades="casos",regio="Total"):
    ibt=[]
    botonets = []
    ordre = 0
    text = ""
    ...
    for chart in list(string.ascii_lowercase):
        ...
        if len_all>7 or chart == "z":
            ...
            s_char = ""
            len_all = 0
            ibt.append(InlineKeyboardButton(text,callback_data="alph_"+str(ordre)))
            ordre +=1
        elif len(result)>0:
            if s_char =="":
                s_char = chart.upper()
...
def b_single(taula,dades="casos",regio="Total"):
    ibt=[]
...
async def show_region(client, user, dataSource = "casos",region = "Total"):
    btns = b_single(comunitat,dades=dataSource,regio=region)
...
async def g_request(client, message):
    ...
    if message.text in comunitat:
        btns = b_single(message.text)
        flname = get_img(message.text)
        caption = get_caption(message.text)+label
        await client.send_photo(user,photo=flname,caption = caption,reply_markup=btns)
